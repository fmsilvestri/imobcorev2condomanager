                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const rows = equipList.map((e, i) => {
                            ...
                            return `<tr style="background:${i%2===0?"#f8fafc":"#ffffff"}"><td>${i+1}</td><td><strong>${e.catIcon} ${e.nome}</strong><br/><span style="color:#6b7280;font-size:10px">${e.modelo||""}</span></td><td>${e.categoria}</td><td>${e.local}</td><td style="color:${stC};font-weight:600">${statusIcon(e.status)} ${e.status}</td><td style="color:${vc};font-weight:600">${pct}%</td><td>${e.proxManutencao||"—"}</td><td style="color:#2563eb">${fv?.nome||"—"}</td></tr>`;
                          }).join("");
                          const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Equipamentos — ${condNome}</title><style>body{font-family:Arial,sans-serif;margin:0;padding:20px;color:#1e293b}h1{font-size:22px;margin-bottom:4px}h2{font-size:16px;color:#475569;margin-bottom:16px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#1e293b;color:#fff;padding:8px 10px;text-align:left}td{padding:7px 10px;border-bottom:1px solid #e2e8f0}.footer{margin-top:24px;font-size:10px;color:#94a3b8;text-align:center}</style></head><body><h1>📋 Equipamentos e Estruturas</h1><h2>${condNome} · Score: <span style="color:${scoreColor}">${scoreEquip}/100</span></h2><table><thead><tr><th>#</th><th>Nome</th><th>Categoria</th><th>Local</th><th>Status</th><th>Vida Útil</th><th>Próx. Manut.</th><th>Fornecedor</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">ImobCore v2 — Plataforma de Gestão Inteligente de Condomínios · ${now}</div></body></html>`;
